...
export async function POST(req: Request) {
  try {
    ...
    const detail_description = formData.get("detail_description")?.toString() || "";
    ...
    if (!detail_description || !detailFiles.length) {
      return NextResponse.json(
        { error: "Missing product detail fields" },
        { status: 400 }
      );
    }
    ...
    const uploadFiles = async (files: File[], folder: string, collected: string[]) => {
    ...
    try {
      // Insert product → LƯU CẢ DETAIL VÀO BẢNG NÀY
      const prodRes = await pool.query(
        `INSERT INTO products (name, price, description, type_id, detail_description, detail_images)
         VALUES ($1,$2,$3,$4,$5,$6)
         RETURNING id`,
        [
          name,
          ...
          type_id,
          detail_description,
          uploadedDetailURLs, // -> Postgres ARRAY
        ]
      );
      ...
      return NextResponse.json(
        { message: "Product + Detail created", product_id: productId },
        { status: 201 }
      );
    } catch (err) {
      ...
      await cleanup(uploadedProductURLs);
      await cleanup(uploadedDetailURLs);
