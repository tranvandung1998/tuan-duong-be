...
export async function POST(req: Request) {
  try {
    ...
    if (!name || !category_id) {
      return new Response(JSON.stringify({ error: 'Missing required fields: name or category_id' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    return new Response(JSON.stringify({ message: 'Type created' }), { status: 201, headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
