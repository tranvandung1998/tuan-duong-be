...
export async function GET(req: Request) {
  ...
  let result;
  if (category) {
    const categoryRes = await pool.query('SELECT id FROM categories WHERE name = $1', [category]);
    if (categoryRes.rowCount === 0) return Response.json({ error: 'Category not found' }, { status: 404 });
...
export async function POST(req: Request) {
  ...
  const catRes = await pool.query('SELECT id FROM categories WHERE name = $1', [category_name]);
  if (catRes.rowCount === 0) return Response.json({ error: 'Category not found' }, { status: 404 });
